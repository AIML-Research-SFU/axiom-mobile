#!/usr/bin/env python3
"""Dataset v3 freeze: deterministic split rebalancing with stratification.

Merges all current entries (pool + val + test) and re-splits them into
balanced pool/val/test sets that reflect the v0.4.0 delta batch (300 new
QA pairs from 60 new screenshots, adding a "cellular signal bars"
question type on top of the v2 set).

Key properties:
  - Deterministic: uses a fixed random seed -> same output every time
  - Stratified: val/test sample from each question-type stratum
  - Versioned: archives v2 manifests, writes SHA256 fingerprint
  - Safe: dry-run mode previews changes without writing

val_count/test_count default to the same absolute sizes used for the v2
freeze (30/40). This is a deliberate choice, not an oversight: the goal
of this freeze is to grow the pool (training data), while keeping the
held-out eval sets the same size so v2 and v3 results stay comparable.
Override with --val-count/--test-count if a larger eval set is wanted.

Usage:
    python3 ml/scripts/freeze_dataset_v3.py                # preview (dry-run)
    python3 ml/scripts/freeze_dataset_v3.py --execute       # write manifests
    python3 ml/scripts/freeze_dataset_v3.py --execute --archive-v2  # archive v2 first
"""
...
# ── Configuration ────────────────────────────────────────────────────
SEED = 20260719          # deterministic seed for reproducibility
VAL_COUNT = 30           # target val size (unchanged from v2, for comparability)
TEST_COUNT = 40          # target test size (unchanged from v2, for comparability)
...
def archive_v2(manifests_dir: Path) -> Path:
    """Archive current (v2) manifests to v2/ subdirectory."""
    v2_dir = manifests_dir / "v2"
    v2_dir.mkdir(exist_ok=True)
    for split in ("pool", "val", "test"):
        src = manifests_dir / f"{split}.jsonl"
        if src.exists():
            shutil.copy2(src, v2_dir / f"{split}.jsonl")
    return v2_dir


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Dataset v3 freeze with deterministic stratified splitting"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    parser.add_argument(
        "--archive-v2",
        action="store_true",
        help="Archive current manifests to data/manifests/v2/ before overwriting",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    print("=" * 60)
    print("  AXIOM-Mobile Dataset v3 Freeze")
    print("=" * 60)
    ...
    print(f"\nProposed v3 split (seed={args.seed}):")
    ...
    # Archive v2
    if args.archive_v2:
        v2_dir = archive_v2(MANIFESTS_DIR)
        print(f"\n[archive] v2 manifests saved to {v2_dir}")
        # Compute v2 fingerprint
        v2_fp = compute_fingerprint(v2_dir)
        fp_path = v2_dir / "fingerprint.json"
        with open(fp_path, "w") as f:
            json.dump({"version": "v2", "sha256": v2_fp}, f, indent=2)
            f.write("\n")
        print(f"  v2 fingerprint: {fp_path}")
    ...
    # Compute and write v3 fingerprint
    v3_fp = compute_fingerprint(MANIFESTS_DIR)
    fp_path = MANIFESTS_DIR / "v3_fingerprint.json"
    with open(fp_path, "w") as f:
        json.dump({
            "version": "v3",
            "seed": args.seed,
        ...
        f.write("\n")
    print(f"\n[fingerprint] {fp_path}")
    for split, sha in v3_fp.items():
        print(f"  {split}: {sha[:16]}...")

    print(f"\nDataset v3 freeze complete.")
    print(f"\nNext steps:")
