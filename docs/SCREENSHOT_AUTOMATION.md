...
- **Dataset v4**: 797 total examples (pool=727, val=30, test=40) — see `data/manifests/v4_fingerprint.json`
- **Auto-generated**: 745 exact-answer examples from 175 screenshots (`exact_v3_batch001` [100, v0.3.0] + `v04_delta_batch001` [60, v0.4.0] + `v05_new_apps_batch001` [15 promoted Safari + 1 Contacts, v0.5.0])
- **Manual examples**: 52 original hand-labeled entries preserved from v1
- **Available apps on iOS 26 simulator**: verified directly (2026-07-31, Xcode 26.6, iOS 26.4/26.5 runtimes) to be far broader than earlier assumed — `xcrun simctl listapps` shows Calendar, Reminders, Contacts, Files, Safari, Photos, Health, News, Weather Wallpaper alongside Settings/Maps. v0.5.0 uses Safari (locally-served synthetic HTML pages, unlimited content variety, zero manual labeling) and Contacts (bundled 6-sample-contact fixture). Reminders was attempted and rejected: its one-time onboarding sheet needs a real tap to dismiss, which this capture harness can't do for system apps. Calendar was rejected: no `simctl` mechanism controls the simulator's actual system clock, so its displayed date can't be made deterministic/reproducible.
- **A real, non-deterministic capture flakiness, documented not hidden**: the first N Safari opens after a fresh app launch land on a transitional "Start Page" or blank page instead of the target content, where N varies run to run (1, 3, and 4 were all observed across different capture attempts) — not a fixed warmup count. `ml/scripts/promote_v05_new_apps.py` uses each capture's recorded `file_size_bytes` as an automated verification gate (the blank/broken band is ~117KB vs ~150-160KB for real rendered text, confirmed against direct visual inspection) rather than trusting a fixed warmup count, so a still-broken capture can never silently reach the manifest.
- **Exact-answer coverage**: status bar (time, battery%, cellular signal bars as of v0.4.0), charging indicator, Apple Account state, Maps search bar text, Safari synthetic page content (order total, item count, status), Contacts sample-fixture count
- **Status bar variants**: 50 time/battery/state combinations (v0.3.0 base) + 30 time/battery/state/cellular_bars combinations (v0.4.0 delta) for visual diversity
- **Rejected as exact answers (v0.4.0 investigation)**: wifi signal bars and carrier/operator name were tested via `simctl status_bar` overrides and visually checked on iPhone 17 Pro / iOS 26.4 — wifi bar count is not reliably distinguishable at this icon resolution, and carrier name is not rendered at all (covered by the Dynamic Island). Only `cellular_bars` was added, since 1-vs-4 bars is unambiguous.
- **iOS 26 note**: Settings main screen no longer shows Airplane Mode, Wi-Fi, or Bluetooth toggles (moved to sub-pages). The existing `ScreenshotCaptureTests.swift` navigation methods for these sub-pages (`testSettingsWiFi`, `testSettingsBluetooth`, etc.) target cells that are not present on the current main screen and are not reachable via deep link either (`App-prefs:` all open Settings main on iOS 26) — sub-page capture remains unimplemented, deferred rather than attempted unreliably.
- **Known data-quality gotcha**: the very first Maps launch in a fresh capture batch can show a one-time "Enable Notifications" onboarding sheet that covers the search bar, making a "What text is shown in the search bar?" answer of "Apple Maps" wrong for that one capture. Confirmed isolated to the first Maps capture per batch (all subsequent captures in the v0.4.0 batch were clean) — spot-check the first Maps screenshot of any new batch before promoting.
- **Dataset versioning**: v1 manifests archived in `data/manifests/v1/`, v2 manifests archived in `data/manifests/v2/`, both with SHA256 fingerprints
...
```bash
python3 scripts/generate_exact_scenarios.py               # full canonical set (reference only)
python3 scripts/generate_exact_scenarios.py --dry-run      # preview
python3 scripts/generate_exact_scenarios.py --delta-only   # only new-since-last-freeze scenarios
```

`--delta-only` writes `capture_scenarios_v04_delta.json` containing only
scenarios not yet captured/promoted. This is what should actually be
passed to `capture_screenshots.sh --scenarios`, since the plain (no-flag)
output includes the full historical scenario set — including scenarios
already promoted into `pool.jsonl` — and re-capturing those would create
duplicate entries (the promoter has no dedup logic; it always appends).

Current scenario set (v0.4.0, delta):
- **30 Settings main variants** x 5 QA pairs = 150 exact entries
- **30 Maps default variants** x 5 QA pairs = 150 exact entries
- **Total: 300 exact QA pairs from 60 screenshots** (adds "How many cellular signal bars are shown?" on top of the v0.3.0 4-question template)

Base scenario set (v0.3.0, already captured/promoted):
...
1. Add a new `_make_<app>_scenario()` function, or extend an existing one with a new optional parameter (see `cellular_bars` on `_make_settings_scenario`/`_make_maps_scenario` for the pattern)
2. Before wiring up a new status-bar or in-app dimension, visually verify it actually renders distinguishably on the target simulator/device — capture two extremes (e.g. min/max) and compare by eye. `wifi_bars` and `operator_name` were tried and rejected this way in v0.4.0; only emit a question if the answer is actually visually verifiable, per this generator's core rule
3. Add new variant tuples to a dedicated `NEW_VARIANTS`-style list rather than editing already-captured variants in place, and add scenarios for them in a `_delta_scenarios()`-style function — this keeps already-promoted scenarios byte-for-byte unchanged
4. Run `python3 scripts/generate_exact_scenarios.py --delta-only` to regenerate just the new scenarios
5. Test with `--dry-run` on the shell script, then capture and visually spot-check a few images (including the *first* capture of any app in the batch — see the Maps notification-prompt gotcha above) before promoting
