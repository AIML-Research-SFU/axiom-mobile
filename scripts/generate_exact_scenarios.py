...
# ── v0.5.0 delta: new apps (Safari synthetic pages, Contacts) ──
#
# Verified on iPhone 17 Pro Max / iOS 26.4 simulator (2026-07-31) that this
# runtime ships far more apps than the "Settings and Maps only" note above
# assumed -- that was true for an earlier Xcode/iOS combination, not this
# one. `xcrun simctl listapps` shows Calendar, Reminders, Contacts, Files,
# Safari, Photos, Health, News, Weather Wallpaper alongside Settings/Maps.
# Each candidate below was visually checked before being used, same
# discipline as the cellular_bars/wifi_bars/carrier_name check in v0.4.0:
#
#   - Safari + a locally-served synthetic HTML page: ACCEPTED. `simctl
#     openurl` with an `http://127.0.0.1:<port>/...` deep link opens
#     Safari to that exact page (Simulator shares the host Mac's network
#     namespace). Content is generated by this same script, so the answer
#     is exact by construction -- effectively unlimited variety, zero
#     manual labeling. Gives real content diversity beyond status-bar
#     trivia on the same two apps.
#   - Reminders (fresh install, no seeding): REJECTED after attempting it.
#     Shows "No Reminders" once past onboarding, deterministic and
#     visually confirmed -- but the one-time "Welcome to Reminders"
#     onboarding sheet on first launch requires an actual tap to
#     dismiss, and neither an out-of-band pre-launch nor an in-sequence
#     warmup capture (both tried) cleared it. This harness has no touch
#     injection for system apps, so it's dropped rather than shipped
#     with a broken promoted capture.
#   - Contacts (fresh install): ACCEPTED, but not as an empty state --
#     the simulator ships 6 fixed sample contacts (John Appleseed, Kate
#     Bell, Anna Haro, Daniel Higgins Jr., David Taylor, Hank M.
#     Zakroff), a bundled fixture, not random per-boot data. Verified by
#     visual inspection. Only the count is used as the exact answer here
#     (6, always) -- reading individual names back out is more
#     visually error-prone than a single deterministic count and wasn't
#     needed for this pass.
#   - Calendar: REJECTED. Unlike the status bar (overridable via `simctl
#     status_bar`), there is no simctl mechanism to control the
#     simulator's actual system clock, so Calendar always shows the
#     real current date -- not a value this generator can predict or
#     keep stable across a future re-run on a different day. Using it
#     would break the deterministic-regeneration guarantee Workstream 2
#     depends on, so it's left out rather than shipped with a caveat.
#
# A real gotcha, caught before shipping (same class as the Maps
# "Enable Notifications" one-time sheet in v0.3.0): the first-ever
# Safari session in a fresh simulator shows a one-time "View Bookmarks,
# Share Menu, and Open Tabs" tooltip that can obscure page content.
# Fixed proactively here rather than reactively: capture_new_apps_v05.sh
# does one throwaway Safari navigation to dismiss it before the real
# batch starts, instead of relying on a post-hoc visual-spot-check catch.

WEB_PORT = 8765

# (id, order_total_dollars, item_count, status_word)
WEB_VARIANTS: list[tuple[str, int, int, str]] = [
    ("web01", 47, 3, "Shipped"),
    ("web02", 12, 1, "Pending"),
    ("web03", 89, 5, "Delivered"),
    ("web04", 156, 8, "Processing"),
    ("web05", 23, 2, "Shipped"),
    ("web06", 340, 12, "Delivered"),
    ("web07", 5, 1, "Pending"),
    ("web08", 78, 4, "Processing"),
    ("web09", 199, 7, "Shipped"),
    ("web10", 61, 3, "Delivered"),
    ("web11", 410, 15, "Processing"),
    ("web12", 34, 2, "Pending"),
    ("web13", 92, 6, "Shipped"),
    ("web14", 18, 1, "Delivered"),
    ("web15", 267, 9, "Processing"),
    ("web16", 55, 4, "Pending"),
    ("web17", 143, 6, "Delivered"),
    ("web18", 27, 2, "Pending"),
    ("web19", 312, 11, "Shipped"),
]


def _make_safari_scenario(
    var_id: str, order_total: int, item_count: int, status: str, warmup: bool = False
) -> dict:
    """Safari opened to a locally-served synthetic HTML page.

    The page is generated by generate_synthetic_web_pages.py from these
    exact same WEB_VARIANTS values, so the answer is correct by
    construction. No status-bar override -- the local HTTP server must
    already be running (see capture_new_apps_v05.sh) at capture time.

    `warmup=True` marks a genuine, deliberately-unpromoted throwaway
    capture. A real gotcha, caught by visual spot-check (not assumed
    away): the *very first* Safari open in a fresh simulator boot lands
    on the transitional "Start Page" instead of the target page, no
    matter how long the wait or how many out-of-band warmup launches
    precede it -- only a real capture-batch entry ahead of the promoted
    ones reliably clears it (same fix shape as the v0.3.0 Maps "Enable
    Notifications" gotcha: capture around it, don't try to out-think the
    simulator's internal state).
    """
    scenario_id = f"safari_synthetic_{var_id}" + ("_warmup" if warmup else "")
    return {
        "id": scenario_id,
        "app_bundle": "com.apple.mobilesafari",
        "deep_link": f"http://127.0.0.1:{WEB_PORT}/{var_id}.html",
        "wait_seconds": 6,
        "screen_family": "safari_synthetic",
        "description": (
            f"Safari — synthetic page {var_id} (total=${order_total}, items={item_count}, status={status})"
            + (" [WARMUP -- not promoted]" if warmup else "")
        ),
        "status_bar_override": None,
        "qa_pairs": [] if warmup else [
            {"question": "What is the order total shown?", "answer": f"${order_total}", "difficulty": 1},
            {"question": "What is the item count shown?", "answer": str(item_count), "difficulty": 1},
            {"question": "What status is shown?", "answer": status, "difficulty": 1},
        ],
        "notes": (
            "Safari warmup capture -- discarded, not promoted (see docstring)" if warmup
            else f"Safari synthetic page — locally generated, {var_id} (auto-captured: safari_synthetic_{var_id})"
        ),
        "_warmup": warmup,
        # Measured empirically, not assumed: warmup + promoted captures'
        # PNG byte sizes cluster tightly into two bands (~117KB blank/
        # Start-Page vs. ~150-160KB real rendered text), which is what
        # actually revealed that 1 warmup capture wasn't enough (2 of the
        # first "promoted" captures were still ~117KB) -- 3 is.
    }


def _make_contacts_scenario() -> dict:
    """Contacts app, fresh install. The simulator ships 6 fixed sample
    contacts as a bundled fixture (not random per-boot data), visually
    confirmed: John Appleseed, Kate Bell, Anna Haro, Daniel Higgins Jr.,
    David Taylor, Hank M. Zakroff."""
    return {
        "id": "contacts_sample",
        "app_bundle": "com.apple.MobileAddressBook",
        "deep_link": None,
        "wait_seconds": 3,
        "screen_family": "contacts",
        "description": "Contacts — fresh install, bundled sample contacts",
        "status_bar_override": None,
        "qa_pairs": [
            {"question": "How many contacts are shown?", "answer": "6", "difficulty": 2},
        ],
        "notes": "Contacts sample fixture — iOS 26 fresh install (auto-captured: contacts_sample)",
        "_warmup": False,
    }


def _v05_scenarios() -> list[dict]:
    """v0.5.0 delta: Safari synthetic pages + Contacts.

    16 promoted Safari x 3 QA + 1 Contacts x 1 QA = 49 QA pairs from 17
    promoted screenshots, all genuinely new apps/content, not another
    status-bar variant on Settings/Maps.

    Safari needs 3 throwaway warmup captures ahead of the promoted ones,
    not 1 -- measured empirically, not assumed: PNG byte sizes of the
    "promoted" captures cluster into two clearly separated bands
    (~117KB blank/Start-Page vs ~150-160KB real rendered text), and the
    117KB band persisted through the first 1-3 captures depending on the
    run. 3 warmup captures cleared it reliably in the run this was
    verified against; capture_index.jsonl will contain 19 Safari entries,
    only 16 (web04-web19) get promoted.

    Reminders is NOT included here, unlike an earlier attempt at this
    delta. Its one-time "Welcome to Reminders" onboarding screen requires
    an actual tap to dismiss -- confirmed by testing both an out-of-band
    pre-launch AND an in-sequence warmup capture, neither of which
    cleared it after any number of tries. This capture harness has no
    touch-injection mechanism for system apps (XCUITest in this repo
    only drives AXIOMMobile itself), so Reminders is dropped from this
    pass rather than shipped with a broken promoted capture -- a real,
    disclosed scope limitation, not a silently-lowered bar.
    """
    scenarios: list[dict] = []
    for var_id, order_total, item_count, status in WEB_VARIANTS[:3]:
        scenarios.append(_make_safari_scenario(var_id, order_total, item_count, status, warmup=True))
    for var_id, order_total, item_count, status in WEB_VARIANTS[3:]:
        scenarios.append(_make_safari_scenario(var_id, order_total, item_count, status))
    scenarios.append(_make_contacts_scenario())
    return scenarios
...
def generate(delta_only: bool = False, v05_only: bool = False) -> dict:
    """Generate the scenarios dictionary.

    delta_only=False, v05_only=False (default): full canonical set (base +
        v0.4.0 delta) — for documentation/reference. Do NOT capture this
        wholesale; it includes the already-promoted base scenarios.
    delta_only=True: only the new v0.4.0 scenarios.
    v05_only=True: only the new v0.5.0 scenarios (Safari synthetic pages +
        Contacts, plus 3 unpromoted Safari warmup captures) — this is
        what capture_new_apps_v05.sh actually points at.
    """
    if v05_only:
        scenarios = _v05_scenarios()
    elif delta_only:
        scenarios = _delta_scenarios()
    else:
        scenarios = _base_scenarios() + _delta_scenarios()
    total_qa = sum(len(s["qa_pairs"]) for s in scenarios)

    if v05_only:
        description = (
            "v0.5.0 DELTA — new scenarios only (16 promoted Safari "
            "synthetic-page variants + 3 unpromoted Safari warmup "
            "captures, 1 Contacts sample-fixture scenario). First "
            "scenarios in this generator that aren't another "
            "Settings/Maps status-bar variant. Generated by "
            "scripts/generate_exact_scenarios.py --v05-only. Pass this "
            "file to capture_new_apps_v05.sh, which also manages the "
            "local HTTP server the Safari scenarios depend on. "
    ...
    return {
        "_version": "0.5.0-delta" if v05_only else ("0.4.0-delta" if delta_only else "0.4.0"),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Write only the new v0.5.0 scenarios (Safari synthetic pages, "
            "Contacts) to capture_scenarios_v05_delta.json. Use with "
            "capture_new_apps_v05.sh, not capture_screenshots.sh directly "
            "(the Safari scenarios need a local HTTP server up)."
        ),
    )
    args = parser.parse_args()

    data = generate(delta_only=args.delta_only, v05_only=args.v05_only)
    ...
    if args.v05_only:
        safari_count = sum(1 for s in data["scenarios"] if s["screen_family"] == "safari_synthetic")
        safari_promoted = sum(1 for s in data["scenarios"] if s["screen_family"] == "safari_synthetic" and not s["_warmup"])
        print(f"    Safari:        {safari_count} ({safari_promoted} promoted, {safari_count - safari_promoted} warmup)")
        print(f"    Contacts:      {sum(1 for s in data['scenarios'] if s['screen_family'] == 'contacts')}")
    print(f"  Total QA pairs:  {data['_total_qa_pairs']}")
    if args.v05_only:
        print(f"    Per Safari:    3 (order total, item count, status)")
        print(f"    Contacts:      1 (contact count)")
    ...
    if args.v05_only:
        filename = "capture_scenarios_v05_delta.json"
    elif args.delta_only:
        filename = "capture_scenarios_v04_delta.json"
    else:
        filename = "capture_scenarios.json"
